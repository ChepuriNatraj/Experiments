...
class ExpenseManager {
    ...
    initThemeToggle() {
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            const next = current === 'neon' ? 'sakura' : 'neon';
            this.theme = next;
            this.applyTheme(next);
            this.saveTheme(next);
            this.toast(`Theme: ${next === 'neon' ? 'Neon' : 'Sakura'}`);
        });
        // Reflect initial label/icon
        this.updateThemeToggleUI();
    }
    
    applyTheme(theme) {
        const body = document.body;
        body.classList.remove('theme-neon', 'theme-sakura');
        body.classList.add(theme === 'neon' ? 'theme-neon' : 'theme-sakura');
        this.updateThemeToggleUI();
        // Manage sakura petals
        if (theme === 'sakura') {
            this.ensureSakuraPetals();
        } else {
            this.clearSakuraPetals();
        }
    }
    ...
    updateThemeToggleUI() {
        ...
        const neonActive = this.theme === 'neon';
        if (label) label.textContent = neonActive ? 'Neon' : 'Sakura';
        if (icon) icon.className = neonActive ? 'ri-flashlight-line' : 'ri-blossom-line';
    }
    ...
    ensureSakuraPetals() {
        ...
        container.classList.add('sakura-petals');
        // Create a modest number of petals for perf
        const petalCount = Math.min(Math.max(Math.floor(window.innerWidth / 35), 18), 40);
        for (let i = 0; i < petalCount; i++) {
            const p = document.createElement('div');
            p.className = 'petal';
            const startX = Math.random() * 100; // vw
            const delay = Math.random() * 8; // s
            const fallDur = 8 + Math.random() * 8; // s
            const swayDur = 3 + Math.random() * 3; // s
